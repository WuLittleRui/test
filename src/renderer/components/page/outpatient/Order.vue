    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
              ...
              <el-breadcrumb-item>预约项目管理</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
        ...
        <OrderEdit ref="OrderEdit" @refresh="getList" /> 
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      HospitalOrderItemApi.getPayList(this.listQuery.page, this.listQuery.limit, this.listQuery.searchKey,this.listQuery.type_id).then(
        data => {
    ...
    delAll() {
      ...
      this.$confirm(
        ...
        await this.multipleSelection.forEach((v, i) => {
          adminIds.push(v.item_id);
        });
        HospitalOrderItemApi.deletePayType(adminIds).then(data => {
          if (data.error === "success") {
            this.$message({
              type: "success",
              message: "删除成功!"
    ...
    handleCreate() {
      this.$nextTick(() => {
        this.$refs["OrderEdit"].showNew();
      });
    },
    //编辑
    handleEdit(index, row) {
      this.$refs["OrderEdit"].showEdit(row.item_id);
    },
    //删除
    handleDelete(index, row) {
      this.$confirm("确认删除" + row.name +"吗？")
        .then(_ => {
          HospitalOrderItemApi.deletePayType(row.item_id).then(data => {
            if (data.error === "success") {
              this.getList();
              this.$notify({
                title: "成功",
