<template>
    <div class="table">
        ...
        <div class="container">
            ...
            <el-table :height="tableheight" :data="list" border class="table" empty-text="没有任何记录" element-loading-text='给我一点时间'
                ...
                <el-table-column prop="groupName" label="岗位名称" header-align="center"  align="center" sortable min-width="120">
                 ...
                 <el-table-column prop="type" label="部门类型" header-align="center"  align="center" sortable min-width="80" :formatter="formatterState">
                </el-table-column>
        ...
        <DepartMentEdit ref="DepartMentEdit" @refresh="getList" /> 
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      HospitalGroupApi.getShopGroupList(
        this.listQuery.page,
 ...
 formatterState(row, column, value) {
    ...
    handleCreate() {
      this.$nextTick(() => {
        this.$refs["DepartMentEdit"].showNew();
      });
    },
    //编辑
    handleEdit(index, row) {
      this.$refs["DepartMentEdit"].showEdit(row.groupId);
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除吗？")
        .then(_ => {
          console.log(row.groupId);
          HospitalGroupApi.deleteShopGroup(row.groupId).then(data => {
            if (data.error === "success") {
              this.getList();
              this.$notify({
                title: "成功",
