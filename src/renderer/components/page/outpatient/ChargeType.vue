<template>
    <div class="table">
        ...
        <ChargeTypeEdit ref="ChargeTypeEdit" @refresh="getList" /> 
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      HospitalChargeTypeApi.getPayList(this.listQuery.page, this.listQuery.limit).then(
        data => {
    ...
    delAll() {
      ...
      this.$confirm(
        ...
        HospitalChargeTypeApi.deletePayType(adminIds).then(data => {
          if (data.error === "success") {
            this.$message({
              type: "success",
              message: "删除成功!"
    ...
    handleCreate() {
      this.$nextTick(() => {
        this.$refs["ChargeTypeEdit"].showNew();
      });
    },
    //编辑
    handleEdit(index, row) {
      this.$refs["ChargeTypeEdit"].showEdit(row.typeId);
    },
    //删除
    handleDelete(index, row) {
      this.$confirm("确认删除" + row.name +"吗？")
        .then(_ => {
          HospitalChargeTypeApi.deletePayType(row.typeId).then(data => {
            if (data.error === "success") {
              this.getList();
              this.$notify({
                title: "成功",
