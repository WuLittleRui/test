<template>
    <div class="table">
        ...
        <PayTypeEdit ref="PayTypeEdit" @refresh="getList" /> 
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      PayTypeApi.getPayList(this.listQuery.page, this.listQuery.limit).then(
        data => {
    ...
    delAll() {
      ...
      this.$confirm(
        ...
        await this.multipleSelection.forEach((v, i) => {
          adminIds.push(v.payTypeId);
        });
        PayTypeApi.deletePayType(adminIds).then(data => {
          if (data.error === "success") {
            this.$message({
              type: "success",
              message: "删除成功!"
    ...
    handleCreate() {
      this.$nextTick(() => {
        this.$refs["PayTypeEdit"].showNew();
      });
    },
    //编辑
    handleEdit(index, row) {
      this.$refs["PayTypeEdit"].showEdit(row.payTypeId);
    },
    //删除
    handleDelete(index, row) {
      this.$confirm("确认删除" + row.payTypeName +"吗？")
        .then(_ => {
          PayTypeApi.deletePayType(row.payTypeId).then(data => {
            if (data.error === "success") {
              this.getList();
              this.$notify({
                title: "成功",
